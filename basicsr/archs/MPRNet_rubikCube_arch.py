...
class Encoder(nn.Module):
    def __init__(self, n_feat, kernel_size, reduction, act, bias, scale_unetfeats, csff):
        ...
        self.encoder_level1_1 = RubikCube_multiply(n_feat, n_feat, 1, 8)
        self.encoder_level2_1 = RubikCube_multiply(n_feat+scale_unetfeats, n_feat+scale_unetfeats, 1, 8)
        self.encoder_level3_1 = RubikCube_multiply(n_feat+(scale_unetfeats*2), n_feat+(scale_unetfeats*2), 1, 8)
    ...
    def forward(self, x, encoder_outs=None, decoder_outs=None):
        enc1 = self.encoder_level1(x) + self.encoder_level1_1(x)
        if (encoder_outs is not None) and (decoder_outs is not None):
            enc1 = enc1 + self.csff_enc1(encoder_outs[0]) + self.csff_dec1(decoder_outs[0])

        x = self.down12(enc1)

        enc2 = self.encoder_level2(x) + self.encoder_level2_1(x)
        if (encoder_outs is not None) and (decoder_outs is not None):
            enc2 = enc2 + self.csff_enc2(encoder_outs[1]) + self.csff_dec2(decoder_outs[1])

        x = self.down23(enc2)

        enc3 = self.encoder_level3(x) + self.encoder_level3_1(x)
        if (encoder_outs is not None) and (decoder_outs is not None):
