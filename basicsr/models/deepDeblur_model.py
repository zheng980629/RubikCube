...
class deepDeblurModel(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(deepDeblurModel, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter):
        ...
        # pixel loss
        if self.cri_pix:
            l_pix1 = self.cri_pix(self.output[0], self.gt)
            l_total += l_pix1
            loss_dict['l_scale1'] = l_pix1

            l_pix2 = self.cri_pix(self.output[1], self.gt[:, :, ::2, ::2])
            l_total += l_pix2
            loss_dict['l_scale2'] = l_pix2

            l_pix3 = self.cri_pix(self.output[2], self.gt[:, :, ::4, ::4])
            l_total += l_pix3
            loss_dict['l_scale3'] = l_pix3
    ...
    def test(self):
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                self.output_test, _, _ = self.net_g_ema(self.lq)
        else:
            self.net_g.eval()
            with torch.no_grad():
                self.output_test, _, _ = self.net_g(self.lq)
            self.net_g.train()
    ...
    def get_current_visuals(self):
        ...
        out_dict['enhanced'] = self.output_test[0]
        if hasattr(self, 'gt'):
